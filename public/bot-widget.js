(function () {
  ...
  class KovalAiElement extends HTMLElement {
    ...
    createWidget() {
      ...
      const handleParentMessage = (event) => {
        ...
        const isImportantMessage =
          ...
          (event.data.type === "USER_DATA_RESPONSE" ||
            ...
            (event.data.intent === "TPA2" &&
              event.data.type === "registerEventListener" &&
              event.data.data &&
              event.data.data.eventKey === "EDIT_MODE_CHANGE"));

        // Only log important messages to reduce noise
        if (isImportantMessage) {
          console.log(
            "📨 Bot widget received message from Wix page:",
            event.data,
          );
        }
        ...
        if (event.data.type === "USER_DATA_UPDATE" && event.data.userData) {
          ...
          if (userData.userId === updatedUserData.userId) {
            ...
            console.log(
              `✅ Updated user data - Dive logs: ${userData.diveLogsCount}, Memories: ${userData.memoriesCount}`,
            );
      ...
      const testNextJSConnection = async () => {
        try {
          ...
          if (response.ok) {
            ...
            if (healthData) {
              console.log(
                `✅ Next.js backend healthy: v${healthData.version || "unknown"}`,
                {
      ...
      const performSystemHealthCheck = async () => {
        try {
          ...
          const response = await fetch(
            `${this.BASE_URL}/api/system/health-check`,
            {
              method: "GET",
              headers: { "Content-Type": "application/json" },
              signal: controller.signal,
          ...
          if (response.ok) {
            ...
            if (health) {
              console.log(`🏥 System health: ${health.status}`, {
                ...
                totalTime: health.totalResponseTime
                  ? `${health.totalResponseTime}ms`
                  : "unknown",
              });
              ...
              if (health.status === "error") {
                console.warn(
                  "❌ System has errors - some features may be limited",
                );
              } else if (health.status === "warning") {
                console.warn(
                  "⚠️ System has warnings - performance may be affected",
                );
              }
          ...
          if (error.name === "AbortError") {
            ...
            console.warn(
              "⚠️ Could not perform system health check:",
              error.message,
            );
          }
          return false;
      ...
      console.log(
        "🔗 Loading embed URL with full app functionality:",
        embedUrl.toString(),
      );
    ...
    handleEditModeChange(isEditMode) {
      console.log(
        `🎛️ Handling edit mode change: ${isEditMode ? "EDIT MODE" : "LIVE MODE"}`,
      );
    ...
    getUserDataFromWix() {
      console.log(
        "🔍 Getting user data from Wix Members/FullData for UserMemory...",
      );
  ...
  console.log(
    "🚀 Koval AI Widget v5.0-DIVELOGS-ENHANCED loaded safely - Cache: " +
      Date.now(),
  );
  console.log("🎯 Widget loaded at: " + loadTime);
  console.log(
    "🔄 V5.0: REAL MEMBER ID DETECTION AND DIVELOGS COLLECTION FIXES APPLIED!",
  );
  console.log(
    "✅ Message types supported: EMBED_READY, CHAT_MESSAGE, SAVE_DIVE_LOG, USER_AUTH, THEME_CHANGE",
  );
  console.log("🛡️ Enhanced error monitoring and timeout handling active");
  console.log(
    "🤖 OpenAI reliability improvements with retry logic and fallbacks",
  );
  console.log(
    "🆔 V5.0: Real Wix Member ID detection (no session-based prefixes)",
  );
  console.log(
    "💾 V5.0: DiveLogs collection save fixes with correct field mapping",
  );
})();
