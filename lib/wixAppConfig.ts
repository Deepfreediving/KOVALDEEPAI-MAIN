...
export const WIX_APP_CONFIG = {
  ...
  makeRequest: async (endpoint: string, options: RequestInit = {}) => {
    ...
    if (!response.ok) {
      throw new Error(
        `Wix App API Error: ${response.status} - ${response.statusText}`,
      );
    }
  ...
  userMemory: {
    ...
    save: async (userId: string, diveLogData: any) => {
      return WIX_APP_CONFIG.makeRequest(WIX_APP_CONFIG.ENDPOINTS.USER_MEMORY, {
        method: "POST",
  ...
  test: {
    ...
    comprehensive: async () => {
      return WIX_APP_CONFIG.makeRequest(
        WIX_APP_CONFIG.ENDPOINTS.COMPREHENSIVE_TEST,
      );
    },
