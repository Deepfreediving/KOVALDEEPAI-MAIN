...
export const createThread = async () => {
  try {
    ...
    // Return the thread ID from the response
    ...
    throw new Error('Error creating thread: ' + (error.message || error.response?.data));  // Improve error message
  }
};
...
export const createMessage = async (threadId, message) => {
  try {
    ...
    const response = await openaiApi.post('/chat/completions', {
      model: process.env.OPENAI_MODEL || 'gpt-4o',  // Use the model set in environment or default to GPT-4o
      messages: [
        {
