...
export const createMessage = async (threadId, message) => {
  try {
    ...
    // Provide more informative error messages for debugging
    throw new Error('Error sending message: ' + (error.message || error.response?.data?.error?.message || 'Unknown error'));
  }
};

// Function to list files uploaded to OpenAI
export const listFiles = async () => {
  try {
    const response = await openaiApi.get('/files');
    console.log('Files in OpenAI system:', response.data); // This will log the list of files and their IDs
  } catch (error) {
