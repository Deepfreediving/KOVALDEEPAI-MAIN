...
const OPENAI_API_URL = process.env.OPENAI_API_BASE || "https://api.openai.com/v1";  // Default OpenAI API base URL if none is provided

// Setup Axios for OpenAI API calls
const openaiApi = axios.create({
  baseURL: OPENAI_API_URL,  // Use the base URL only once
  headers: {
    'Authorization': `Bearer ${process.env.OPENAI_API_KEY}`,  // Authorization header with the API key from the environment
...
export const createThread = async () => {
  try {
    const response = await openaiApi.post('/chat/completions', {
      model: process.env.OPENAI_MODEL || 'gpt-4',  // Default model set to GPT-4 if not provided in the environment
      messages: [
    ...
    if (!response.data || typeof response.data.id !== 'string') {
      const errorMessage = 'Thread creation failed: No valid threadId returned.';
...
export const createMessage = async (threadId, message) => {
  try {
    ...
    const response = await openaiApi.post('/chat/completions', {
      model: process.env.OPENAI_MODEL || 'gpt-4',  // Use the model set in environment or default to GPT-4
      messages: [
        {
