...
export default async function handler(
  ...
  try {
    console.log(
      "🔍 Testing connection to @deepfreediving/kovaldeepai-app backend...",
    );
  ...
  try {
    ...
    const localResponse = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/analyze/save-dive-log`,
      {
        method: "POST",
