...
export default async function handler(
  ...
  if (!fs.existsSync(tokensPath)) {
    return res
      .status(404)
      .json({ authenticated: false, message: "No tokens found" });
  }
  ...
  try {
    ...
    console.error("❌ Failed to parse token file:", err);
    fs.unlinkSync(tokensPath);
    return res
      .status(500)
      .json({ authenticated: false, message: "Invalid token file" });
  }
  ...
  if (isExpired && tokens.refreshToken) {
    try {
      ...
      return res
        .status(500)
        .json({ authenticated: false, error: "Token refresh failed" });
    }
  }
