...
export default async function handler(
  ...
  try {
          ...
          for (const subFile of subFiles) {
            if (subFile.endsWith(".json")) {
              try {
                ...
                console.warn(
                  `Warning: Could not parse ${file}/${subFile}:`,
                  error,
                );
              }
            }
    ...
    try {
      ...
      const wixFetchResponse = await fetch(
      ...
      if (wixFetchResponse.ok) {
        ...
        if (wixData.success && wixData.data) {
          ...
          console.log(
            `✅ Fetched ${wixLogs.length} logs from Wix diveLogs collection`,
          );
        }
      }
      ...
      const unSyncedLogs = localLogs.filter(
        (log: LocalLog) => !log.syncedToWix,
      );

      for (const log of unSyncedLogs) {
        try {
          console.log(
            `🔄 Uploading log ${log.id} to Wix diveLogs collection...`,
          );

          // ✅ CORRECTED: Use diveLogs endpoint, not userMemory
          const wixUploadResponse = await fetch(
          ...
          if (wixUploadResponse.ok) {
            ...
            if (uploadData.success) {
              ...
              uploadedCount++;
              console.log(
                `✅ Uploaded log ${log.id} to Wix diveLogs collection`,
              );
            }
          }
        } catch (uploadError) {
          console.warn(
            `⚠️ Failed to upload log ${log.id}:`,
