...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`🔍 Querying Wix collection: ${collectionId} with filter:`, filter);

    // ✅ OPTION 1: Try direct Wix backend function if available
    try {
      const backendResponse = await fetch('https://www.deepfreediving.com/_functions/queryData', {
        method: 'POST',
        headers: { 
      ...
      if (backendResponse.ok) {
        ...
        console.log(`✅ Wix backend query success: ${backendData.items?.length || 0} items in ${processingTime}ms`);
    ...
    if (process.env.WIX_API_KEY) {
      try {
        ...
        if (response.ok) {
          console.log(`✅ Wix Data API query success: ${data.items?.length || 0} items in ${processingTime}ms`);
    ...
    const processingTime = Date.now() - startTime;
    console.log(`📝 No data found for collection ${collectionId} after ${processingTime}ms`);
