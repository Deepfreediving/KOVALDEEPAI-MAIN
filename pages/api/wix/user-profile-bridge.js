  ...
  try {
    ...
    try {
      ...
      if (membersResponse.ok) {
        ...
        if (membersData.items && membersData.items.length > 0) {
          ...
          console.log(
            `✅ Members/FullData profile loaded in ${processingTime}ms`,
          );
    ...
    try {
      ...
      if (membersResponse.ok) {
        ...
        if (membersData.items && membersData.items.length > 0) {
          ...
          console.log(
            `✅ Members collection profile loaded in ${processingTime}ms`,
          );
      ...
      if (queryResponse.ok) {
        ...
        if (queryData.items && queryData.items.length > 0) {
          ...
          try {
            ...
            console.log(
              `✅ DiveLogs collection profile loaded in ${processingTime}ms`,
            );
            ...
            console.warn(
              "⚠️ Could not parse profile from DiveLogs:",
              parseError,
            );
          }
        }
    ...
    try {
      const wixMasterResponse = await fetch(
        "https://www.deepfreediving.com/_functions/http-getUserProfile",
        {
          method: "POST",
          headers: {
      ...
      console.warn(
        "⚠️ Wix backend profile unavailable:",
    ...
    return res.status(200).json({
      ...
      source: "default-profile",
      message:
        "Created default profile. Complete your profile for personalized coaching!",
      metadata: {
        processingTime,
        isDefault: true,
        sourcesChecked: [
