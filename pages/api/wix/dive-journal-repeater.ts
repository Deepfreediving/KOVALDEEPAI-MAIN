...
async function saveDiveJournalToRepeater(req: NextApiRequest, res: NextApiResponse) {
  const {
    userId,
    title,
    date,
...
async function getDiveJournalFromRepeater(req: NextApiRequest, res: NextApiResponse) {
  const { userId, limit = 20, analysisStatus, needsPatternAnalysis } = req.query;
  ...
  try {
    ...
    if (wixResponse.ok) {
      ...
      const diveJournalEntries = wixData.data?.filter((item: any) => 
        item.discipline && item.reachedDepth !== undefined
      ) || [];

      console.log(`✅ Loaded ${diveJournalEntries.length} dive journal entries from repeater`);
      ...
      console.error('❌ Wix UserMemory repeater query failed:', wixResponse.status);
      return res.status(500).json({
        success: false,
