...
export default async function handler(req, res) {
  ...
  try {
    const { userMessage, conversationHistory, userId, profile, embedMode = true } = req.body;
    ...
    if (userId && !userId.startsWith('guest')) {
      try {
        ...
        if (diveLogsResponse.ok) {
          ...
          console.log(`📊 Loaded ${diveLogs.length} dive logs for enhanced coaching`);
        }
      } catch (diveError) {
        console.warn('⚠️ Could not load dive logs for context:', diveError.message);
      }
    }

    // ✅ OPTION 1: Try consolidated Wix chat endpoint (original name)
    try {
      const wixMasterResponse = await fetch('https://www.deepfreediving.com/_functions/http-chat', {
        method: 'POST',
        headers: { 
      ...
      if (wixMasterResponse.ok) {
        ...
        console.log(`✅ Wix master chat response received in ${processingTime}ms`);
        ...
        console.warn(`⚠️ Wix master endpoint returned ${wixMasterResponse.status}`);
      }
    } catch (wixError) {
    ...
    try {
      const wixLegacyResponse = await fetch('https://www.deepfreediving.com/_functions/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userMessage,
      ...
      if (wixLegacyResponse.ok) {
        ...
        console.log(`✅ Wix legacy chat response received in ${processingTime}ms`);
      ...
      console.warn('⚠️ Wix legacy backend also unavailable:', wixLegacyError.message);
    }
    ...
    if (nextjsResponse.ok) {
      ...
      console.log(`✅ Next.js fallback response received in ${processingTime}ms`);
    ...
    return res.status(500).json({ 
      ...
      aiResponse: "⚠️ I'm having technical difficulties right now. Please try again in a moment, and I'll be happy to help with your freediving training!",
      success: false,
      metadata: { 
        processingTime,
