...
async function syncToWix(
  ...
  const WIX_SITE_URL =
    process.env.WIX_SITE_URL || "https://www.deepfreediving.com";
...
export default async function handler(
  ...
  try {
          ...
          if (fs.existsSync(logFile)) {
            try {
              const logData = JSON.parse(
                await fs.promises.readFile(logFile, "utf8"),
              );
    ...
    const wixUserIdResponse = await axios.get(
      `${process.env.WIX_SITE_URL || "https://www.deepfreediving.com"}/_functions/getUserId`,
      {
        headers: {
          "Content-Type": "application/json",
          "User-Agent": "KovalAI/1.0",
    ...
    if (diveLogId && userId) {
      ...
      if (!userId) {
        ...
        await fetch(
          `${process.env.NEXTAUTH_URL || "http://localhost:3000"}/api/analyze/record-memory`,
          {
            method: "POST",
