...
(function() {
  ...
  async function testBackendConnection() {
    ...
    try {
      const response = await fetch(TEST_CONFIG.baseUrl + TEST_CONFIG.endpoints.wixConnection);
  ...
  async function testChatEndpoint(userId) {
    ...
    try {
      const response = await fetch(TEST_CONFIG.baseUrl + TEST_CONFIG.endpoints.chat, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
          userMessage: 'Hello, this is a test message. Can you help me with freediving?',
          userId: userId,
  ...
  async function testUserMemoryEndpoint(userId) {
    ...
    try {
      ...
      // Test POST
      const postResponse = await fetch(TEST_CONFIG.baseUrl + TEST_CONFIG.endpoints.userMemory, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  async function runAllTests() {
    ...
    console.log('User Authentication:', results.userAuth.authenticated ? '✅ AUTHENTICATED' : '⚠️ GUEST');
    console.log('Widget Loading:', results.widgetLoading ? '✅ PASS' : '⚠️ NO WIDGET');
    console.log('Chat Endpoint:', results.chatEndpoint ? '✅ PASS' : '❌ FAIL');
    console.log('User Memory:', results.userMemoryEndpoint ? '✅ PASS' : '❌ FAIL');

    const passCount = Object.values(results).filter(r => r === true || r?.authenticated).length;
  ...
  console.log('✅ Test system loaded. Run window.KovalAITests.runAllTests() to start testing.');
