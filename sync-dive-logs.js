...
async function getAuthenticatedUserId() {
    try {
        ...
        const response = await fetch('https://www.deepfreediving.com/_functions/getUserProfile', {
            method: 'GET',
            headers: {
...
async function syncDiveLogsToWix() {
    ...
    try {
        ...
        for (const file of diveLogFiles) {
            try {
                ...
                if (diveLog.userId && diveLog.userId !== USER_ID) {
                    console.log(`⏭️ Skipping ${file} - belongs to different user (${diveLog.userId})`);
                    skippedCount++;
                    continue;
                }
                ...
                console.log(`📝 Processing: ${diveLog.discipline} dive on ${diveLog.date} (${diveLog.reachedDepth}m)`);
        ...
        console.log(`✅ Sync completed! ${syncedCount}/${diveLogFiles.length} dive logs synced to Wix`);
        console.log(`⏭️ Skipped: ${skippedCount} files (wrong user or missing data)`);
        console.log('🔍 Check your UserMemory collection in Wix CMS to verify the data');
        console.log('\n🎯 Next steps:');
        console.log('1. Test API: curl "http://localhost:3000/api/analyze/dive-logs?userId=' + USER_ID + '"');
