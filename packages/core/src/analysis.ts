...
export class AIAnalysisService {
  ...
  async analyzeDiveLog(request: AnalysisRequest): Promise<AnalysisResult> {
    try {
  ...
  async analyzeImage(imageBase64: string, diveContext?: Partial<DiveLog>): Promise<string> {
    try {
      const completion = await this.openai.chat.completions.create({
        model: "gpt-4o",
        messages: [
  ...
  async generateProgressReport(diveLogs: DiveLog[], timeframe: string = '30 days'): Promise<string> {
    try {
