...
async function testMemberDataRetrieval() {
  ...
  try {
    ...
    const testUserId = wixUsers.currentUser.loggedIn
  ...
  try {
    const response = await fetch("/_functions/chat", {
      ...
      body: JSON.stringify({
        ...
        userId: wixUsers.currentUser.loggedIn
          ? wixUsers.currentUser.id
          : "test-user",
