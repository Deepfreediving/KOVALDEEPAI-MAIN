...
async function testBackendEndpoints() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log(`⚠️ wixConnection: ${response.status} - May still be deploying`);
        }
    } catch (error) {
    ...
    try {
        const response = await fetch(`${baseUrl}/_functions/memberProfile?userId=${testUserId}`);
        if (response.ok) {
            console.log('✅ memberProfile: SUCCESS - Should return rich profile data');
        } else {
            console.log(`⚠️ memberProfile: ${response.status} - May still be deploying`);
        }
    } catch (error) {
    ...
    console.log('✅ Real user names (like "Daniel Koval") instead of "Authenticated User"');
