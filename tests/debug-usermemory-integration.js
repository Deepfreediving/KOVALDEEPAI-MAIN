...
async function testBasicConnectivity() {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/wix/dive-journal-repeater?userId=test-connectivity`, {
      method: 'GET',
      headers: {
...
async function testRetrieval(userId = 'debug-user-123') {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/wix/dive-journal-repeater?userId=${userId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    if (response.ok) {
      const result = JSON.parse(text);
      console.log('✅ Retrieval successful:', result.logs?.length || 0, 'logs found');
      return result.logs;
    } else {
