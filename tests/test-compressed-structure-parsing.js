...
console.log('🔍 Testing Compressed Structure Parsing (GET function simulation)\n');
...
function parseCompressedDiveLogData(mockCollectionItems) {
  console.log(`📋 Processing ${mockCollectionItems.length} compressed records...`);
...
const mockDiveLogData = {
  ...
  logEntry: JSON.stringify({
    ...
    analysis: {
      ...
      technicalNotes: "Mouthfill at 15m | Issue: Slight equalization difficulty | Surface: Standard recovery",
...
try {
  ...
  if (parsedData.diveLogs.length > 0) {
    ...
    console.log('Mouthfill Depth:', diveLog.logEntry.dive?.depths?.mouthfill + 'm');
    console.log('Exit Quality:', diveLog.logEntry.dive?.performance?.exit);
    console.log('Notes:', diveLog.logEntry.dive?.notes);
    
    console.log('\n🤖 AI Analysis Data:');
    console.log('===================');
    console.log('Progression Score:', diveLog.logEntry.analysis?.progressionScore + '%');
    console.log('Depth Achievement:', diveLog.logEntry.analysis?.depthAchievement + '%');
    console.log('Risk Factors:', diveLog.logEntry.analysis?.riskFactors?.join(', ') || 'None');
