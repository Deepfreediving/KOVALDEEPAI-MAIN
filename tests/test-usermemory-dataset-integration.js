...
async function testUserMemoryDatasetIntegration() {
  console.log(
    "🧪 Testing UserMemory-@deepfreediving/kovaldeepai-app/Import1 dataset integration...\n",
  );
  ...
  try {
    ...
    const loadResponse = await fetch(
      `${API_BASE}/api/wix/dive-journal-repeater?userId=${testDiveLog.userId}&limit=10`,
    );
    ...
    if (saveResult.wixId) {
      ...
      if (analysisResponse.ok) {
        ...
        console.log("✅ Individual analysis successful:", {
          ...
          hasRecommendations:
            analysisResult.analysis?.includes("recommend") || false,
        });
      } else {
    ...
    if (patternResponse.ok) {
      ...
      console.log("✅ Pattern analysis successful:", {
        ...
        hasInsights: !!(
          ...
          Object.keys(patternResult.insights).length > 0
        ),
      });
    } else {
    ...
    const chatResponse = await fetch(`${API_BASE}/api/openai/chat`, {
      ...
      body: JSON.stringify({
        message:
          "Analyze my recent dive performance and suggest improvements for my CNF training.",
        userId: testDiveLog.userId,
        profile: {
    ...
    console.log(
      "🔄 All dive logs should now appear in your Wix repeater connected to this dataset.",
    );
    console.log("🤖 Click any log in the sidebar for instant AI analysis.");
    console.log(
      "🔬 Systematic pattern analysis available for advanced coaching insights.",
    );
  } catch (error) {
