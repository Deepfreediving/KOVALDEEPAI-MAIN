...
async function testCompleteDiveLogFlow() {
  ...
  const diveLogData = {
    ...
    notes:
      "Good dive, felt comfortable at depth. Need to work on equalization below 25m.",
    totalDiveTime: "",
    issueComment: "",
  ...
  try {
    const logsResponse = await axios.get(
      `${API_BASE}/api/analyze/get-dive-logs?userId=${userId}`,
    );
    console.log(
      "✅ Dive logs loaded:",
      logsResponse.data.diveLogs?.length || 0,
      "logs found",
    );
  ...
  const chatData = {
    message:
      "Can you analyze my most recent dive log and give me coaching feedback?",
    userId,
    embedMode: true,
    profile: userProfile,
    threadId: null,
  ...
  try {
    ...
    const hasDepthMention =
      response.includes("28") ||
      response.includes("30") ||
      response.includes("depth");
    const hasLocationMention =
      response.includes("blue hole") || response.includes("location");
    const hasNotesMention =
      response.includes("equalization") || response.includes("25m");
