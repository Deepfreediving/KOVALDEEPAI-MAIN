...
async function testWixRepeaterIntegration() {
  ...
  const testDiveLog = {
    ...
    notes: 'Excellent dive, good equalization down to 30m, slight pressure at 33m but managed well. Exit felt comfortable.',
    totalDiveTime: '',
    issueComment: '',
  ...
  try {
    const saveResponse = await axios.post(`${API_BASE}/api/wix/dive-journal-repeater`, {
      ...testDiveLog,
    ...
    console.error('❌ Failed to save to Wix repeater:', error.response?.data || error.message);
  }

  // Test 2: Retrieve dive journals from repeater
  console.log('\n📊 Step 2: Retrieving dive journals from Wix UserMemory repeater...');
  try {
    const getResponse = await axios.get(`${API_BASE}/api/wix/dive-journal-repeater?userId=${userId}&limit=10`);
    
    console.log('✅ Retrieved dive journals from repeater:', getResponse.data);
    console.log(`📋 Found ${getResponse.data.entries?.length || 0} dive journal entries`);
    ...
    console.error('❌ Failed to retrieve from Wix repeater:', error.response?.data || error.message);
  }

  // Test 3: Individual dive log analysis
  console.log('\n🔍 Step 3: Testing individual dive log analysis...');
  try {
    const analysisResponse = await axios.post(`${API_BASE}/api/analyze/single-dive-log`, {
      userId,
    ...
    console.error('❌ Individual analysis failed:', error.response?.data || error.message);
  }
  ...
  try {
    const chatResponse = await axios.post(`${API_BASE}/api/openai/chat`, {
      message: 'Can you analyze my recent dive performance and give me coaching feedback for my next training session?',
      userId,
      embedMode: true,
      profile: {
    ...
    console.error('❌ Chat integration failed:', error.response?.data || error.message);
  }
