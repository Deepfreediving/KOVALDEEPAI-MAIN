...
async function testEmbeddedWidget() {
  ...
  try {
    ...
    await page.goto(
      "http://localhost:3000/embed?userId=test-integration&nickname=IntegrationTest&theme=light&embedded=true",
    );

    // Wait for the widget to load
    await page.waitForSelector(
      '.chat-container, .sidebar, [data-testid="chat-input"]',
      { timeout: 10000 },
    );
    ...
    const userDisplayElements = await page.$$eval("*", (elements) => {
      return elements
        .filter(
          (el) => el.textContent && el.textContent.includes("IntegrationTest"),
        )
    ...
    try {
      ...
      const diveJournalExists = await page.$(
        '[data-testid="dive-journal"], .dive-journal, input[name="date"], input[placeholder*="date"]',
      );
      ...
      if (diveJournalExists) {
        ...
        const dateInput = await page.$(
          'input[name="date"], input[type="date"], input[placeholder*="date"]',
        );
        if (dateInput) {
        ...
        const disciplineSelect = await page.$(
          'select[name="discipline"], select[name="disciplineType"]',
        );
        if (disciplineSelect) {
          await page.select(
            'select[name="discipline"], select[name="disciplineType"]',
        ...
        const submitButton = await page.$(
          'button[type="submit"], button:contains("Submit"), button:contains("Save")',
        );
        if (submitButton) {
        ...
        console.log(
          "ℹ️ Dive journal form not found (may be in sidebar or requires interaction)",
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (chatInput) {
        await chatInput.type(
          "Hello, can you help me with my freediving training?",
        );
        console.log("✅ Chat message typed");

        // Look for send button
        const sendButton = await page.$(
          'button[type="submit"], button:contains("Send"), [data-testid="send-button"]',
        );
        if (sendButton) {
          ...
          const messages = await page.$$eval(
            '.message, .chat-message, [data-role="assistant"]',
            (elements) => {
              return elements
                .map((el) => el.textContent.trim())
...
try {
  ...
  console.log(
    "🔧 Alternative: Manually test at http://localhost:3000/embed?userId=test&nickname=TestUser&embedded=true",
  );
}
