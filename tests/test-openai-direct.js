        ...
        messages: [
          {
            role: "system",
            content:
              "You are Koval AI, a professional freediving coach. Provide helpful, safety-focused advice.",
          },
          {
            role: "user",
...
async function testLocal() {
  ...
  try {
    ...
    console.log(
      "- Content:",
      data.assistantMessage?.content?.substring(0, 150) + "...",
    );

    if (data.metadata?.fallbackUsed) {
      console.log(
        "⚠️ FALLBACK WAS USED - Original Error:",
        data.metadata.originalError,
      );
    }
...
async function runTests() {
  ...
  console.log(
    `Local Chat Endpoint: ${localWorks ? "✅ Working" : "❌ Using Fallback"}`,
  );
