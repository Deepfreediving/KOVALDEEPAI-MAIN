...
async function testDiveLogLoading() {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/analyze/get-dive-logs?userId=sim-user-123`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
...
async function testDiveLogAnalysis(userId = 'sim-user-123') {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('🤖 AI insight preview:', data.analysis?.substring(0, 150) + '...');
      console.log('📊 Updated progression score:', data.updatedLog?.progressionScore);
...
async function testPatternAnalysis(userId = 'sim-user-123') {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('📊 Key insights:', data.insights?.keyFindings?.join(', ') || 'None');
      console.log('🎯 Recommendations:', data.insights?.recommendations?.slice(0, 2)?.join(', ') || 'None');
...
async function runFrontendIntegrationTest() {
  ...
  console.log('💡 This tests the dive log flow as it would work from your Wix frontend\n');
  ...
  console.log('🤖 Individual analysis:', analysis ? 'Working' : 'Needs OpenAI config');
  console.log('📈 Pattern analysis:', patterns ? 'Working' : 'Needs OpenAI config');
  console.log('\n💡 Next steps:');
  ...
  console.log('3. Test from your live Wix page at https://www.deepfreediving.com/large-koval-deep-ai-page');
}
