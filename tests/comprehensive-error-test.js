...
async function testMultipleDiveLogs() {
  ...
  try {
    const response = await fetch(
      `${API_BASE}/api/analyze/get-dive-logs?userId=error-test-user`,
    );
    if (response.ok) {
      const result = await response.json();
...
async function testIndividualAnalysis(diveIds, logs) {
  ...
  console.log(
    `🎯 Testing analysis of dive: ${testLog.location} (${testLog.targetDepth}m)`,
  );
...
async function runComprehensiveErrorTests() {
  ...
  console.log("==========================");
  console.log(
    `✅ Multiple dive logs: ${results.multipleDives?.success ? "PASS" : "FAIL"}`,
  );
  console.log(
    `✅ Individual analysis: ${results.individualAnalysis?.success ? "PASS" : "FAIL"}`,
  );
  console.log(
    `✅ Wix error handling: ${results.wixErrors?.success ? "PASS" : "FAIL"}`,
  );
  console.log(
    `✅ Pattern analysis: ${results.patternAnalysis?.success ? "PASS" : "FAIL"}`,
  );
