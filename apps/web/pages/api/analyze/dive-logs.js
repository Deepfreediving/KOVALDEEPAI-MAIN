  ...
  try {
    ...
    if (userId) {
      ...
      const baseDir = isServerless
        ? "/tmp/diveLogs"
        : path.join(process.cwd(), "data", "diveLogs");
        ...
        for (const file of userFiles) {
          if (file.endsWith(".json")) {
            try {
              ...
              console.warn(
                `⚠️ Could not parse user log ${file}:`,
                parseError.message,
              );
            }
          }
      ...
      for (const file of files) {
        ...
        if (stat.isFile() && file.endsWith(".json")) {
          try {
            ...
            console.warn(
              `⚠️ Could not parse general log ${file}:`,
              parseError.message,
            );
          }
          ...
          try {
            ...
            for (const subFile of subFiles) {
              if (subFile.endsWith(".json")) {
                ...
                try {
                  ...
                  diveLog.lastModified = fs
                    .statSync(subFilePath)
                    .mtime.toISOString();

                  allDiveLogs.push(diveLog);
                } catch (parseError) {
                  console.warn(
                    `⚠️ Could not parse dive log file ${file}/${subFile}:`,
                    parseError.message,
                  );
                }
              }
            ...
            console.warn(
              `⚠️ Could not read subdirectory ${file}:`,
              subDirError.message,
            );
          }
        }
      ...
      try {
        ...
        if (bridgeResponse.ok) {
          ...
          if (bridgeData.success && bridgeData.diveLogs?.length > 0) {
            ...
            console.log(
              `✅ Loaded ${bridgeData.diveLogs.length} logs from DiveLogs collection`,
            );
          }
        }
      } catch (bridgeError) {
        console.log(
          "ℹ️ DiveLogs collection not available, using file fallback:",
          bridgeError.message,
        );
      }
    }
    ...
    allDiveLogs.sort((a, b) => {
      ...
      const priorityA =
        priorityOrder[a.source] || priorityOrder[a.priority] || 1;
      const priorityB =
        priorityOrder[b.source] || priorityOrder[b.priority] || 1;
    ...
    if (userId && userId !== "undefined" && !userId.startsWith("guest-")) {
      ...
      if (userLogs.length === 0) {
        ...
        userLogs = allDiveLogs.filter((log) => {
          return (
            log.filePath &&
            (log.filePath.includes(userId) || log.id?.includes(userId))
          );
        });
      }
      ...
      userLogs = userLogs.filter((log) => {
        ...
        if (userId.match(/^[a-f0-9-]{36}$/)) {
          ...
          if (log.userId !== userId) {
            console.log(
              `🚫 SECURITY: UUID user ${userId} - filtering out log with different userId: ${log.userId || "undefined"}`,
            );
            return false;
          }
        }
        ...
        if (
          ...
          console.log(
            `🚫 Filtering out test/guest log: ${log.id} (userId: ${log.userId})`,
          );
          return false;
        }
        ...
        if (
          ...
          console.log(
            `🚫 Filtering out test directory log: ${log.filePath} (id: ${log.id})`,
          );
          return false;
        }
      ...
      console.log(
        `✅ Found ${filteredLogs.length} verified logs for userId ${userId}`,
      );
    } else {
      console.log(
        `⚠️ No valid userId provided (${userId}), returning empty for security`,
      );
      filteredLogs = [];
    }

    console.log(
      `✅ Loaded ${filteredLogs.length} dive logs for user ${userId || "unknown"}`,
    );
    ...
    if (filteredLogs.length > 0) {
      ...
      console.log(
        `📊 Logs with userId field: ${logsWithUserId}/${filteredLogs.length}`,
      );
