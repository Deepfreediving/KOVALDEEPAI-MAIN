...
export default async function handler(
  ...
  try {
    ...
    try {
      ...
      results.openai.embedding = embedding?.data?.length
        ? "✅ Embedding generated"
        : "❌ Failed";
    } catch (err: any) {
    ...
    try {
      ...
      await index.upsert([
        { id: "verify-test-1", values: vector, metadata: { text: "verify" } },
      ]);
    ...
    return res
      .status(500)
      .json({ status: "error", message: error.message || "Unknown error" });
  }
}
