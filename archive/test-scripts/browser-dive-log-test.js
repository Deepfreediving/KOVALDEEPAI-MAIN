...
if (window.KOVAL_USER_DATA_V5) {
  ...
  try {
    ...
    if (userLogs.length > 0) {
      ...
      userLogs.slice(0, 3).forEach((log, i) => {
        console.log(
          `   ${i + 1}. ${log.date} - ${log.location || "No location"} - ${log.reachedDepth || log.targetDepth || "No depth"}m`,
        );
      });
    }
  } catch (e) {
  ...
  console.log(
    "   • Look for legacy KOVAL_USER_DATA:",
    !!window.KOVAL_USER_DATA,
  );
}
...
const iframes = document.querySelectorAll(
  'iframe[src*="kovaldeepai-main.vercel.app"]',
);
...
if (iframes.length > 0) {
  ...
  console.log(
    "   • Contains V5.0 params:",
    iframes[0].src.includes("v=") ? "✅ Yes" : "❌ No",
  );
  console.log(
    "   • Contains userId param:",
    iframes[0].src.includes("userId=") ? "✅ Yes" : "❌ No",
  );
} else {
...
console.log(
  "2. If duplicates found: V5.0 deduplication should prevent new ones",
);
...
console.log("\n📋 Test summary available as window.testSummary");
