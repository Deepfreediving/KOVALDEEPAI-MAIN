  ...
  try {
    ...
    if (result.success) {
      ...
      const checkResponse = await fetch(
        `/api/analyze/get-dive-logs?userId=${testLog.userId}`,
      );
  ...
  for (const collectionName of possibleCollections) {
    try {
      ...
      if (result.success && !result.error) {
        console.log(
          `✅ ${collectionName}: Accessible, ${result.items?.length || 0} items`,
        );
      } else {
        console.log(
          `❌ ${collectionName}: ${result.error || "Not accessible"}`,
        );
      }
    } catch (error) {
...
function checkEnvironmentSetup() {
  ...
  console.log(
    ...
    typeof process !== "undefined"
      ? "***" + (process.env?.WIX_API_KEY?.slice(-4) || "undefined")
      : "browser-context",
  );
  console.log(
    "- BASE_URL:",
...
async function testWixBackendDirect() {
  ...
  try {
    ...
    const response = await fetch(
      "https://www.deepfreediving.com/_functions/dive-journal-repeater",
      {
        method: "POST",
        headers: {
...
async function runEmergencyDiagnostics() {
  ...
  console.log(
    "2. If all APIs fail, check Wix collection exists and has correct permissions",
  );
