...
if (hasV5UserData) {
  ...
  checkResult(
    "V5.0 Version",
    userData.version === "v5.0-DIVELOGS-ENHANCED",
    `Version: ${userData.version || "unknown"}`,
  );
}

// 2. Check localStorage dive logs
const diveLogKeys = Object.keys(localStorage).filter((key) =>
  key.includes("diveLogs"),
);
checkResult(
  "LocalStorage Keys",
  ...
  diveLogKeys.forEach((key) => {
    try {
      ...
      if (ids.length !== uniqueIds.length) {
        hasDuplicates = true;
        console.log(
          `⚠️ Duplicates in ${key}: ${logs.length - uniqueIds.length} duplicates`,
        );
      }
    } catch (e) {
...
window.quickDiveLogTest = function () {
  ...
  try {
    ...
    console.log(
      success ? "✅" : "❌",
      `Save test: ${beforeCount} → ${afterLogs.length} logs`,
    );
...
console.log(
  "\n📋 Verification results available as window.verificationResults",
);
