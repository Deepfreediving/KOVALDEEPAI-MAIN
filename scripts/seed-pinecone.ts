...
// Multiple data directories to process
const DATA_DIRS = [
  path.join(process.cwd(), "data", "koval-lessons"),
  path.join(process.cwd(), "data", "experience levels")
];
...
async function main() {
  for (const DATA_DIR of DATA_DIRS) {
    if (!fs.existsSync(DATA_DIR)) {
      console.log(`⚠️ Directory not found: ${DATA_DIR}`);
      continue;
    }
    ...
    if (files.length === 0) {
      console.log(`⚠️ No .txt files found in ${path.basename(DATA_DIR)} folder.`);
      continue;
    }

    console.log(`📁 Processing directory: ${path.basename(DATA_DIR)}`);
