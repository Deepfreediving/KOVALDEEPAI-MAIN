    ...
    try {
        ...
        for (const file of jsonFiles) {
            try {
                ...
                console.log(`   Dive: ${diveLog.discipline || 'Unknown'} - ${diveLog.reachedDepth || 'Unknown'}m at ${diveLog.location || 'Unknown location'}`);
        ...
        if (processedFiles.length > 0) {
            ...
            processedFiles.forEach(({ file, discipline, depth, location, date }) => {
                console.log(`   • ${discipline} - ${depth}m at ${location} (${date}) - ${file}`);
            });
        }
        
        console.log('\n🎯 Next steps:');
        console.log('1. Test the API: curl "http://localhost:3000/api/analyze/dive-logs?userId=' + YOUR_USER_ID + '"');
