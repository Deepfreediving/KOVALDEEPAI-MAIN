...
console.log('- WIX_API_KEY:', process.env.WIX_API_KEY ? `***${process.env.WIX_API_KEY.slice(-4)}` : 'NOT SET');
    ...
    if (response.ok) {
      try {
        ...
        if (diveLogs) {
          ...
          console.log('💡 You need to create a DiveLogs collection in your Wix site');
        }
      } catch (e) {
