...
export function ensureLogDir() {
  try {
    ...
    console.warn(
      "⚠️ Could not create log directory, will use memory storage:",
      error,
    );
    return false;
  }
...
export async function saveLogEntry(
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        "✅ Dive log saved to DiveLogs collection:",
        result.data?._id,
      );
...
export async function deleteLogEntry(userId: string, logId: string) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        wixDeleted
          ? "✅ Deleted from Wix DiveLogs collection"
          : "⚠️ Wix deletion failed",
      );
    } else {
