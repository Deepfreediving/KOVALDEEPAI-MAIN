...
function testDiveLogSaveNow() {
    console.log('🔍 Testing dive log save to DiveLogs collection with correct field names...');
    ...
    const testDiveLog = {
        ...
        targetDepth: 15,
        reachedDepth: 15,
        notes: 'FIELD MAPPING TEST - If you see this in DiveLogs collection, the field bug is FIXED!',
    ...
    if (typeof wixData !== 'undefined' && wixData.save) {
        console.log('✅ Wix data API found - testing direct save with correct field names...');
        ...
        wixData.save('DiveLogs', directTestData)
            .then(result => {
                console.log('🎉 DIRECT SAVE SUCCESS! Field mapping fix worked:', result._id);
    ...
    if (typeof saveDiveLogToWix === 'function') {
        ...
        saveDiveLogToWix(testDiveLog)
            .then(result => {
                console.log('🎉 WIDGET SAVE SUCCESS! Dive log saved successfully:', result);
                console.log('✅ The DiveLogs collection save is working!');
                console.log('🔍 Now check your Wix Editor > Database > DiveLogs collection');
...
console.log('4. If test succeeds, try saving a real dive log through the widget');
console.log('================================================');
